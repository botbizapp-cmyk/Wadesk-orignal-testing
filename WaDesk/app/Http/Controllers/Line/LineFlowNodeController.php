...
/**
 * Laravel callbacks for the Node LINE flow engine
 * (node/services/lineFlowService.js). Node runs every SEND itself via the
 * Messaging API and calls back here only to mirror a flow message into the
 * inbox (flow-log) and to resolve "smart" nodes — AI / webhook (flow-node).
 * Both are X-Node-Token guarded. Mirrors TelegramFlowNodeController.
 */
class LineFlowNodeController extends Controller
{
    ...
    /** POST /api/line/flow-log — mirror an outbound flow message into the inbox. */
    public function log(Request $request): JsonResponse
    {
        ...
        $channel   = $channelId > 0 ? LineChannel::find($channelId) : null;

        if ($channel && $userId !== '' && $body !== '') {
            $wsId = (int) $channel->workspace_id;
            $conv = Conversation::firstOrCreate(
                ['workspace_id' => $wsId, 'channel' => 'line', 'raw_jid' => 'line:'.$channel->id.':'.$userId],
                ['title' => 'LINE', 'provider' => 'line', 'origin' => 'line', 'status' => 'pending', 'inbox_status' => 'open', 'last_message_at' => now(), 'contact_digits' => null]
            );
            try {
                ...
                Log::warning('[LINE-FLOW-NODE] flow-log write failed: '.$e->getMessage());
            }
        }
    ...
    /** POST /api/line/flow-node — resolve a smart node (ai / webhook). */
    ...
    private function handleAi(Request $request): JsonResponse
    {
        ...
        $system = trim((string) ($node['system'] ?? $node['systemPrompt'] ?? $node['prompt'] ?? ''))
            ?: 'You are a helpful assistant replying inside a LINE conversation. Be concise and friendly.';
        ...
        if ($assistantId > 0 && $wsId > 0) {
            try {
                ...
                Log::warning('[LINE-FLOW-NODE] ai knowledge-base inject failed: '.$e->getMessage());
            }
        }
    ...
    private function handleWebhook(Request $request): JsonResponse
    {
        ...
        try {
            ...
            Log::warning('[LINE-FLOW-NODE] webhook request failed: '.mb_substr($e->getMessage(), 0, 200));
        }
