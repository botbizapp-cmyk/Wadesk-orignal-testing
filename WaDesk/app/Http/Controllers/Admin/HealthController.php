...
class HealthController extends Controller
{
    ...
    /**
     * Clear orphaned queue rows. This platform is deliberately queue-less —
     * every broadcast event is ShouldBroadcastNow and there is NO queue worker
     * (repo policy: no jobs/queues/workers/cron). So anything sitting in the
     * `jobs` table is a stale broadcast left over from an older build that no
     * worker will EVER drain — pure dead weight that trips the Queue card to
     * "degraded". Deleting it is always safe and instantly returns the card to
     * green. One-click admin action; no worker/cron involved.
     */
    public function clearQueue(Request $request): JsonResponse
    {
        ...
        try {
            ...
            return response()->json(['ok' => false, 'message' => $this->short($e)], 500);
        }
        ...
        return response()->json([
            ...
            'message'        => "Cleared {$jobs} pending and {$failed} failed job(s).",
        ]);
    }
