...
class CheckoutSettingsController extends Controller
{
    public function index()
    {
        ...
        return view('admin.checkout-settings.index', [
            ...
            // Invoice logo — current uploaded one (if any) for the preview, plus
            // the effective URL invoices will actually print (own logo → brand
            // logo fallback).
            'hasInvoiceLogo'     => (string) SystemSetting::get('billing.logo', '') !== '',
            'invoiceLogoUrl'     => \App\Support\Brand::invoiceLogoUrl(),
        ]);
    }
    ...
    public function update(Request $request)
    {
        $data = $request->validate([
            ...
            // Invoice logo — printed on every platform invoice/receipt.
            'company_logo'          => ['nullable', 'image', 'mimes:png,jpg,jpeg,svg,webp', 'max:1024'],
            'remove_logo'           => ['nullable', 'in:0,1,on'],
        ]);
        ...
        // Invoice logo. A new upload replaces the old path; ticking "remove"
        // clears it (invoices then fall back to the site brand logo, then the
        // company wordmark). Stored as a public-disk path — Brand::resolveUrl()
        // turns it into asset('storage/…').
        if (!empty($data['remove_logo'])) {
            SystemSetting::set('billing.logo', '', 'string');
        } elseif ($request->hasFile('company_logo')) {
            $path = $request->file('company_logo')->store('billing', 'public');
            SystemSetting::set('billing.logo', $path, 'string');
        }
