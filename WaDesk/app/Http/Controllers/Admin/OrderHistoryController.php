...
class OrderHistoryController extends Controller
{
    ...
    /**
     * Cancel a stuck PENDING order (e.g. an abandoned gateway checkout that
     * never completed). The awaiting-approval panel's Approve/Reject only
     * covers offline payment-proof orders, so a pending gateway order that was
     * never paid had no row action at all — the reported "can't cancel or
     * update pending payment". This marks it failed (cancelled) so it stops
     * counting as outstanding/overdue and drops out of the pending KPIs.
     */
    public function cancel(Request $request, int $id)
    {
        $order = Order::findOrFail($id);

        // Never cancel money that already changed hands — a paid/refunded row
        // must go through the proper refund path, not a silent status flip.
        if ($order->status === 'paid') {
            return back()->with('info', 'Order ' . $order->order_number . ' is already paid — cancel is only for pending orders.');
        }
        if ($order->status !== 'pending') {
            return back()->with('info', 'Order ' . $order->order_number . ' is not pending.');
        }
        ...
        \App\Support\Audit::log('admin.order.cancelled', ['layer' => 'platform', 'meta' => [
            'order'    => $order->order_number,
            'amount'   => $order->amount,
            'currency' => $order->currency,
        ]]);
