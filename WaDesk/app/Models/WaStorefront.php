...
class WaStorefront extends Model
{
    ...
    /**
     * Human label for the store's SENDING number, resolving BOTH engines:
     *   - device_id set  → the bound Unofficial (Baileys) device's phone.
     *   - device_id NULL → the workspace's connected OFFICIAL (WABA/Twilio)
     *     sender. A store on the official API stores device_id = null by design
     *     (the send routes via WorkspaceEngine::for()), so the card must resolve
     *     the Cloud-API number instead of showing "No device" (the reported bug:
     *     a WABA store showed "No device" because the view only did Device::find).
     * Returns null only when nothing is connected.
     */
    public function getSendingLabelAttribute(): ?string
    {
        if ($this->device_id) {
            $d = Device::find($this->device_id);
            if ($d) {
                $ph = trim(($d->country_code ? '+' . $d->country_code . ' ' : '') . ($d->phone_number ?? ''));
                return $ph !== '' ? $ph : ($d->device_name ?: ('Device #' . $d->id));
            }
        }
        // Official (WABA/Twilio) fallback — the store's device_id is null when it
        // sends over the Cloud API; show the workspace's connected official number.
        $cfg = \App\Models\WaProviderConfig::query()
            ...
            ->first();
        if ($cfg) {
            $ph = trim((string) $cfg->phone_number);
            return $ph !== '' ? $ph : ($cfg->display_label ?: 'Official API (WABA)');
        }
        return null;
    }
