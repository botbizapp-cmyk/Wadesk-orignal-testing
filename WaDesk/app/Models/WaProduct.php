...
class WaProduct extends Model
{
     ...
     */
    /**
     * Convert our internally-stored price_minor to the integer Meta's catalog
     * items_batch `price` field expects — the amount in the currency's OWN
     * minor unit.
     *
     * Our price_minor is ALWAYS stored ×100 (a 2-decimal convention) regardless
     * of currency. Meta wants:
     *   • 2-decimal (USD, INR, EUR…) → minor = major×100 → send price_minor as-is
     *   • ZERO-decimal (IDR, JPY, KRW, VND, HUF…) → minor = major → send price_minor/100
     *   • 3-decimal (KWD, BHD, OMR…) → minor = major×1000 → send price_minor×10
     *
     * Without the zero-decimal case, IDR Rp 395,000 (price_minor 39,500,000) was
     * pushed to Meta as 39,500,000 and shown as Rp 39.500.000 — exactly 100× too
     * high. Same rule as the Meta Ads budget multiplier (MetaGraphClient).
     */
    public static function metaCatalogPrice(int $priceMinor, ?string $currency): int
    {
        $cur = strtoupper((string) ($currency ?: 'INR'));
        if (in_array($cur, ['BIF','CLP','DJF','GNF','IDR','ISK','JPY','KMF','KRW','MGA','PYG','RWF','UGX','VND','VUV','XAF','XOF','XPF','HUF','TWD'], true)) {
            return intdiv($priceMinor, 100);   // zero-decimal → strip our internal ×100
        }
        if (in_array($cur, ['BHD','IQD','JOD','KWD','LYD','OMR','TND'], true)) {
            return $priceMinor * 10;           // 3-decimal → major×1000 (price_minor is major×100)
        }
        return $priceMinor;                    // 2-decimal → already the right minor units
    }
    ...
    public function toMetaCatalogPayload(string $shopUrl = ''): array
    {
        ...
        return array_filter([
            ...
            'price'                    => static::metaCatalogPrice((int) $this->price_minor, $this->currency_code),
