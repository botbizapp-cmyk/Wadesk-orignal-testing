...
class FormatSettings
{
    ...
    {
        // When no workspace is passed, resolve the SIGNED-IN user's active
        // workspace so money renders in THEIR currency (an India user sees ₹,
        // an Indonesia user sees Rp) — not the platform default. Guarded for
        // CLI / webhook / unauthenticated contexts where there is no user.
        if (!$workspace) {
            try {
                if (auth()->check()) {
    ...
    /**
     * Symbol (or code) for a SPECIFIC currency code — the per-code companion to
     * symbol(). Used where the money isn't the platform default, e.g. a
     * storefront's own currency on the /store chart legend (which hard-coded ₹
     * regardless of the workspace's actual currency). Falls back to the bare
     * code when the row has no symbol, and '' for a null code.
     */
