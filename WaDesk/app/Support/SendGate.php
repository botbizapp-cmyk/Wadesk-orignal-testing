...
class SendGate
{
    ...
    {
        // Wallet gate (bulk paths). Stop a campaign / broadcast / flow blast
        // before it starts when the workspace wallet is empty — delivery-time
        // billing settles the true cost, this just prevents giving away a whole
        // batch for free. Runs regardless of the security guardrails mode.
        try {
            $wsId = (int) ($ctx['workspace_id'] ?? 0);
            if ($wsId > 0) {
                $ws = \App\Models\Workspace::find($wsId);
                if ($ws) {
                    $chk = app(\App\Services\MessageBillingService::class)->precheck($ws, (int) ($ctx['count'] ?? 1));
                    if (! $chk['ok']) {
                        self::audit('billing.send_blocked_no_wallet', $chk['reason'] ?? 'wallet empty', $ctx);
                        throw new \RuntimeException($chk['reason'] ?? 'Your message wallet is empty. Top up to start sending.');
                    }
                }
            }
        } catch (\RuntimeException $e) {
            throw $e; // the block itself — propagate to the caller
        } catch (\Throwable $e) {
            Log::warning('[SENDGATE] wallet precheck errored, allowing: ' . $e->getMessage());
        }
