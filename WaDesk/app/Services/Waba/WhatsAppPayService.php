...
class WhatsAppPayService
{
    ...
    public function applyPaymentWebhook(int $workspaceId, array $value): void
    {
        ...
        try {
            ...
            if ($mapped === 'captured') {
                // AUTO-INVOICE — a WA-Pay capture is a paid native order, but it sets
                // status='processing' and never fires WaOrderPaid, so neither the
                // sweeper (wants paid/completed) nor the paid-order listener would
                // invoice it. Trigger it directly here. handleWebhookOrder gates on
                // auto_send_own + the auto_invoice plan + dedup; issue-only, the
                // sweeper renders + sends the PDF. Best-effort — never break settle.
                try {
                    app(\App\Services\Invoice\InvoiceService::class)
                        ->handleWebhookOrder($order, 'own', null, 'wa_pay');
                } catch (\Throwable $e) {
                    Log::warning('[WAPAY] auto-invoice failed: ' . $e->getMessage());
                }
