...
class TemplateLinter
{
    ...
    public function check(WaTemplate $t): array
    {
        ...
        if ($t->template_type === 'catalog')  $this->checkCatalog($t, $errors, $warnings);
    ...
    private function checkBody(WaTemplate $t, array &$errors, array &$warnings): void
    {
        ...
        // Start/end-with-placeholder: a STRONG recommendation, not a hard block.
        // Meta approves many templates that end with a variable (utility "…confirmed
        // for {{1}}", OTP-style, etc.), so this is advisory — warn, but let the user
        // submit and let Meta be the final judge instead of blocking a template Meta
        // might well accept.
        $trimmed = trim($body);
        if (preg_match('/^\{\{\s*[\w_]+\s*\}\}/', $trimmed)) {
            $warnings[] = 'Body starts with a placeholder. Meta may reject this — safer to add text before {{1}} (e.g. "Hi {{1}}, …"). You can still submit and let Meta decide.';
        }
        if (preg_match('/\{\{\s*[\w_]+\s*\}\}\s*$/', $trimmed)) {
            $warnings[] = 'Body ends with a placeholder. Meta may reject this — safer to add closing text after the last {{N}}. You can still submit and let Meta decide.';
        }
        // Consecutive placeholders, run-on spaces, and tabs ARE reliable
        // automatic-rejection triggers, so these stay hard errors.
    ...
    /**
     * Catalog templates (Meta official, WABA-only). Meta's hard rules for a
     * CATALOG-button template — every one is a create-time reject if broken, so
     * they block the submit here instead of round-tripping to Meta:
     *   - category MUST be MARKETING
     *   - exactly ONE button, and it must be the CATALOG ("View catalog") button
     *     — no URL / phone / quick-reply / OTP button may accompany it
     *   - no media or location header
     * Plus a send-time prerequisite surfaced as a warning (Meta is the final
     * judge, and the catalog may be linked on Meta's side even if not synced
     * locally): a product catalog connected to the WABA.
     */
    private function checkCatalog(WaTemplate $t, array &$errors, array &$warnings): void
    {
        if (strtoupper((string) $t->meta_category) !== 'MARKETING') {
            $errors[] = 'Catalog templates must use the MARKETING category.';
        }

        $btns        = is_array($t->buttons) ? $t->buttons : [];
        $catalogBtns = array_filter($btns, fn ($b) => strtolower((string) ($b['type'] ?? '')) === 'catalog');
        $otherBtns   = array_filter($btns, fn ($b) => strtolower((string) ($b['type'] ?? '')) !== 'catalog');

        if (count($catalogBtns) === 0) {
            $errors[] = 'Catalog template is missing its “View catalog” button.';
        } elseif (count($catalogBtns) > 1) {
            $errors[] = 'A catalog template can have only one “View catalog” button.';
        }
        if (count($otherBtns) > 0) {
            $errors[] = 'A catalog template cannot mix the “View catalog” button with any other button (URL, phone, or quick reply). Remove the other buttons.';
        }

        $att = strtolower((string) ($t->attachment_type ?? ''));
        if ($att !== '' && $att !== 'none') {
            $errors[] = 'Catalog templates cannot have a media or location header — remove the attachment.';
        }
        if (trim((string) $t->header) !== '') {
            $warnings[] = 'Catalog templates are sent without a header — the header text will be ignored by WhatsApp.';
        }

        $wsId = (int) ($t->workspace_id ?? 0);
        if ($wsId && class_exists(\App\Models\WaCatalog::class)) {
            $hasCatalog = \App\Models\WaCatalog::where('workspace_id', $wsId)->where('provider', 'waba')->exists();
            if (! $hasCatalog) {
                $warnings[] = 'No product catalog is connected to your WhatsApp Business account yet. Connect a catalog so the “View catalog” button has products to show.';
            }
        }
    }
