...
<section class="bg-paper-50 hairline-t hairline-b" data-fc-section="feature-connectivity">
    <div class="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-7 py-28">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
            ...
            <div class="lg:col-span-3 flex flex-col justify-end pb-3">
                ...
                <div class="text-[13px] font-semibold" data-fc="feature-connectivity.label">
                    {{ fc('feature-connectivity.label', __('Connectivity · every channel')) }}</div>
            </div>
        ...
        <p class="text-[15.5px] text-ink-700 max-w-2xl leading-relaxed reveal" style="--d:120ms"
            ...
            {{ fc('feature-connectivity.body', __("Run WhatsApp, Instagram, Facebook, Telegram, TikTok and SMS from one shared inbox — the same flows, templates and agents across every channel. And for WhatsApp itself, connect three ways: official Cloud API, Twilio, or scan a QR to test in two minutes.")) }}
        </p>

        {{-- Omni-channel strip — every channel the platform runs, one inbox. --}}
        <div class="mt-7 flex flex-wrap gap-2 reveal" style="--d:140ms">
            @foreach (['WhatsApp', 'Instagram', 'Facebook', 'Telegram', 'TikTok', 'SMS'] as $ch)
                <span class="pill bg-white hairline text-ink-700 text-[12px]">{{ $ch }}</span>
            @endforeach
        </div>
