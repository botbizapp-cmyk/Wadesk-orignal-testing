                    ...
                    @forelse ($orders as $o)
                        ...
                        <tr class="hover:bg-paper-50/60 {{ $rowBg }}">
                            ...
                            <td class="px-3 py-3 text-center">
                                ...
                                @endif
                                {{-- A stuck/abandoned pending order (no payment proof to review in
                                     the panel above) can be settled or cancelled straight from the row,
                                     so it stops sitting in Outstanding/Overdue forever. --}}
                                @if ($o->status === 'pending')
                                    <div class="mt-1.5 flex flex-col items-stretch gap-1">
                                        <form method="POST" action="{{ route('admin.order-history.approve', $o->id) }}"
                                            onsubmit="return confirm('{{ __('Mark this order as paid and activate the plan?') }}');">
                                            @csrf
                                            <button type="submit"
                                                class="w-full inline-flex items-center justify-center gap-1 px-2 py-1 rounded-full bg-wa-green/10 text-wa-deep hover:bg-wa-green/20 text-[10px] font-semibold whitespace-nowrap">
                                                <svg viewBox="0 0 16 16" class="w-3 h-3 shrink-0" fill="none"
                                        ...
                                        <form method="POST" action="{{ route('admin.order-history.cancel', $o->id) }}"
                                            onsubmit="return confirm('{{ __('Cancel this pending order? It will be marked failed and drop out of outstanding.') }}');">
                                            @csrf
                                            <button type="submit"
                                                class="w-full inline-flex items-center justify-center gap-1 px-2 py-1 rounded-full hairline border border-accent-coral/40 text-accent-coral hover:bg-accent-coral/10 text-[10px] font-semibold whitespace-nowrap">
                                                <svg viewBox="0 0 16 16" class="w-3 h-3 shrink-0" fill="none"
                                                    stroke="currentColor" stroke-width="2">
                                                    <path d="M4 4l8 8M12 4l-8 8" />
                                                </svg>
