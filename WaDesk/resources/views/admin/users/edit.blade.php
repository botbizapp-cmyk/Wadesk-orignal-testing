<x-layouts.admin :title="__('Admin · Edit user')" admin-key="users" page="admin-users-edit">
                    ...
                    <div>
                        ...
                        <label for="avatarInput"
                            ...
                            <span id="avatarPreviewWrap"
                                class="w-12 h-12 rounded-full bg-paper-100 grid place-items-center text-ink-500 overflow-hidden shrink-0">
                                @if ($user->avatar_url)
                                    <img id="avatarPreview" src="{{ $user->avatar_url }}"
                                        class="w-full h-full object-cover" alt="">
                                @else
                                    <img id="avatarPreview" src="" alt="" class="w-full h-full object-cover hidden">
                                    <svg id="avatarPlaceholder" viewBox="0 0 16 16" class="w-5 h-5" fill="none"
                            ...
                            <div class="flex-1 min-w-0">
                                <div id="avatarName" class="text-[12px] font-semibold truncate">{{ __('Upload image') }}
                                </div>
    ...
    {{-- Avatar picker preview — shows the chosen filename + a live image preview
         once a file is selected. The picker itself opens via the <label for>, so
         this is purely the visual confirmation. --}}
    <script>
        (function () {
