...
<x-layouts.user :title="__('New Broadcast')" nav-key="more" page="user-broadcasts-create">
    ...
    <main class="max-w-none mx-auto px-4 sm:px-6 lg:px-7 py-6">
        ...
        @php $isIg = ($broadcastChannel ?? 'whatsapp') === 'instagram'; @endphp
        <form id="broadcastForm" method="POST" action="{{ route('user.broadcasts.store') }}"
            ...
            @csrf
            @if ($isIg)<input type="hidden" name="channel" value="instagram">@endif
            <section class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card overflow-hidden">
                <div class="px-5 py-4 border-b border-paper-200 bg-paper-50/40">
                    <div class="flex items-center gap-2.5">
                        ...
                        <span class="font-mono text-[10px] text-ink-500">{{ $isIg ? __('direct message') : __('template message') }}</span>
                    </div>
                </div>
                ...
                <div class="p-5 space-y-5">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        @if ($isIg)
                        <div>
                            <label class="text-[11.5px] font-semibold text-ink-700 flex items-center justify-between gap-2 mb-1.5" for="igAccount">{{ __('Send from') }} <span class="text-accent-coral">*</span></label>
                            <select id="igAccount" name="ig_account_id" class="w-full px-3 py-2 border border-paper-200 rounded-lg bg-white text-[12.5px] text-ink-900 leading-snug focus:outline-none focus:border-wa-deep focus:ring-4 focus:ring-wa-deep/10">
                                @forelse ($igAccounts as $a)
                                    <option value="{{ $a['id'] }}">{{ $a['handle'] }}</option>
                                @empty
                                    <option value="">{{ __('No Instagram account connected') }}</option>
                                @endforelse
                            </select>
                            <div class="text-[10.5px] text-ink-500 mt-1">{{ __('The Instagram account this DM is sent from.') }}</div>
                        </div>
                        @else
                        <div>
                            <label
...
// marketing/utility/auth sends don't get downgraded.
                            ...
                            {{-- Send-time mapping: edit this broadcast's variable
 values, header media, and button links WITHOUT editing the
 template. Posts `template_overrides`. --}}
                            <x-template-live-mapping :templates="$templates" target="#templateSelect" />
                    ...
                    @if ($isIg)
                    {{-- Content — choose "Write text" or "Use template" (a saved
                         Instagram message that prefills the box). --}}
                    <div>
                        <label class="text-[11.5px] font-semibold text-ink-700 flex items-center justify-between gap-2 mb-1.5" for="igMessage">{{ __('Message') }} <span class="text-accent-coral">*</span></label>

                        <div class="inline-flex rounded-lg border border-paper-200 p-0.5 mb-2 bg-paper-50" role="tablist">
                            <button type="button" data-igtype="text" class="igtype-btn px-3.5 py-1.5 rounded-md text-[12px] font-semibold">{{ __('Write text') }}</button>
                            <button type="button" data-igtype="template" class="igtype-btn px-3.5 py-1.5 rounded-md text-[12px] font-semibold">{{ __('Use template') }}</button>
                        </div>

                        <div id="igTemplateWrap" class="hidden mb-2">
                            @if (($igTemplates ?? collect())->count())
                                <select id="igTemplate" data-ig-templates='@json($igTemplates)'
                                    class="w-full px-3 py-2 border border-paper-200 rounded-lg bg-white text-[12.5px] focus:outline-none focus:border-wa-deep">
                                    <option value="">{{ __('— Pick a saved template —') }}</option>
                                    @foreach ($igTemplates as $t)
                                        <option value="{{ $t['id'] }}">{{ $t['name'] }}@if (count($t['buttons'])) · {{ count($t['buttons']) }} {{ __('button(s)') }}@endif</option>
                                    @endforeach
                                </select>
                                <div class="text-[10.5px] text-ink-500 mt-1">{{ __('Picking a template fills the message below — you can still edit it.') }}</div>
                            @else
                                <div class="text-[11px] text-ink-500 border border-paper-200 rounded-lg px-3 py-2 bg-paper-50">
                                    {{ __('No Instagram templates yet.') }} <a href="{{ url('/templates/create') }}" class="text-wa-deep font-semibold hover:underline">{{ __('Create one →') }}</a>
                                </div>
                            @endif
                        </div>

                        <textarea id="igMessage" name="message" rows="4" maxlength="1000" required
                            placeholder="{{ __('Hi {name}! …') }}"
                            class="w-full px-3 py-2 border border-paper-200 rounded-lg bg-white text-[12.5px] text-ink-900 leading-snug resize-y focus:outline-none focus:border-wa-deep focus:ring-4 focus:ring-wa-deep/10">{{ old('message') }}</textarea>
                        <div class="text-[10.5px] text-ink-500 mt-1">{{ __('Instagram sends a plain DM — no approval needed. Use {name} to personalise.') }}</div>
                    </div>
                    @endif
 ...
 the audience. --}}
                    ...
                    <div class="border-t border-paper-200 pt-5">
                        <div class="flex items-center justify-between gap-4 mb-3">
                            <div>
                                <h2 class="font-serif text-[20px] leading-tight">
                                    {{ $isIg ? __('Which Instagram chats?') : __('Who do you want to send it to?') }}</h2>
                        ...
                        @unless ($isIg)
                        {{-- Tag audience — ticking a tag adds every contact carrying
 it, merged with groups / individual picks / manual numbers.
 Same resolution + workspace guard as the campaigns page. --}}
                        @if (!empty($tags) && count($tags))
                            <div class="mb-3 flex flex-wrap gap-2">
                                <span
                                    class="font-mono text-[10px] uppercase tracking-[0.14em] text-ink-500 mr-1 self-center">{{ __('Tags') }}</span>
                                @foreach ($tags as $tg)
                                    <label
                                        ...
                                        <input type="checkbox" name="tags[]" value="{{ $tg->id }}" class="accent-wa-deep"
                                        ...
                                        <span>{{ $tg->name }}</span>
                                        <span
                                            class="font-mono text-[10px] text-ink-500">{{ $tagCounts[$tg->id] ?? 0 }}</span>
                                    </label>
                                @endforeach
                            </div>
                        @endif
                        ...
                        <div class="relative mb-3">
                            ...
                            <input id="contactSearch" type="text"
                                ...
                                placeholder="{{ $isIg ? __('Search by name') : __('Search by name or phone') }}">
                        </div>
                                ...
                                <table class="w-full text-[12.5px] table-fixed">
                                    ...
                                    <tbody class="divide-y divide-paper-200" id="contactsTbody">
                                        @if ($isIg)
                                            @php
                                                $igRows = [];
                                                foreach (($igChats ?? []) as $acctId => $rows) {
                                                    foreach ($rows as $r) { $igRows[] = $r + ['account' => (string) $acctId]; }
                                                }
                                            @endphp
                                            @forelse ($igRows as $r)
                                                <tr data-search="{{ mb_strtolower($r['title']) }}" data-ig-account="{{ $r['account'] }}">
                                                    <td class="px-4 py-2.5"><input data-contact type="checkbox" name="recipients[]" value="{{ $r['id'] }}" class="w-4 h-4 accent-wa-deep"></td>
                                                    <td class="px-3 py-2.5"><div class="font-semibold truncate">{{ $r['title'] }}</div></td>
                                                    <td class="px-3 py-2.5 font-mono text-[11px] text-ink-400">{{ __('Instagram DM') }}</td>
                                                </tr>
                                            @empty
                                                <tr><td colspan="3" class="px-4 py-6 text-center text-[12px] text-ink-500">{{ __('No Instagram conversations yet — someone must message this account first.') }}</td></tr>
                                            @endforelse
                                        @else
            ...
            <aside class="space-y-4">
                <div class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card p-4 sticky top-[92px]">
                    <div class="flex items-center justify-between mb-3">
                        <span
                            class="font-mono text-[10px] uppercase tracking-[0.16em] text-ink-500">{{ $isIg ? __('Message preview') : __('Template preview') }}</span>
                    ...
                    <div class="rounded-[24px] border border-ink-900/10 bg-ink-900 p-2 shadow-soft">
                        <div class="rounded-[19px] overflow-hidden bg-wa-chat">
                            <div class="h-11 bg-wa-deep text-paper-0 flex items-center gap-2 px-3">
                                <span
                                    class="w-7 h-7 rounded-full bg-paper-0 text-wa-deep grid place-items-center text-[11px] font-semibold">{{ $isIg ? 'IG' : 'WA' }}</span>
                                <div class="min-w-0">
                                    <div class="text-[12px] font-semibold truncate">{{ __('Broadcast preview') }}
                                    </div>
                                    <div class="text-[9.5px] text-paper-100 truncate">{{ $isIg ? __('Instagram DM') : __('Template message') }}
                                    </div>
                                </div>
                            </div>
                            <div
 ...
 carries an attachment_type. Mirrors the builder's #pp-attach. --}}
                                    ...
                                    <p id="previewBody"
                                        ...
                                        {{ $isIg ? __('Your message preview…') : __('Pick a template to preview…') }}</p>
