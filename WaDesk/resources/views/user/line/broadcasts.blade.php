...
<x-layouts.user :title="__('LINE broadcasts')" nav-key="more" page="user-line-broadcasts">

    <main class="max-w-none mx-auto px-4 sm:px-6 lg:px-7 py-7 space-y-6">

        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div class="min-w-0">
                <div class="font-mono text-[10px] uppercase tracking-[0.18em] text-ink-500 mb-2">{{ __('LINE') }} · {{ __('Broadcasts') }}</div>
                <h1 class="font-serif font-normal tracking-tight text-[32px] sm:text-[40px] leading-none">{{ __('LINE') }} <span class="italic" style="color:#06C755">{{ __('broadcasts') }}</span></h1>
                <p class="text-[13px] text-ink-600 mt-2 max-w-xl">{{ __('Message everyone who has added and chatted with your LINE account, then track sent, failed and blocked per broadcast.') }}</p>
            </div>
            <div class="flex items-center gap-2 shrink-0">
                ...
                @unless ($channels->isEmpty())
                    <a href="{{ route('user.line.broadcasts.create') }}" class="px-4 py-2 rounded-full text-white text-[12px] font-semibold inline-flex items-center gap-2" style="background:#06C755">
        ...
        @else
            ...
            <div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
                ...
                <div class="bg-paper-0 border border-paper-200 rounded-2xl p-4 shadow-card">
                    ...
                    <div class="mt-2 text-[11px] text-ink-500">{{ __('blocked by user') }}</div>
                </div>
            ...
            <div class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-[12.5px]" id="line-broadcasts" data-poll="{{ route('user.line.broadcasts.index') }}">
                        <thead>
                            <tr class="text-left text-ink-500 border-b border-paper-200 bg-paper-50/50">
                                ...
                                <th class="px-5 py-2.5 font-mono text-[10px] uppercase tracking-wide">{{ __('Channel') }}</th>
                        ...
                        <tbody>
                            @forelse ($broadcasts as $b)
                                ...
                                <tr class="border-b border-paper-100 last:border-0 hover:bg-paper-50/60" data-bcast="{{ $b->id }}">
                                    ...
                                    <td class="px-5 py-3 text-[11.5px] text-ink-600 whitespace-nowrap">{{ $b->channel?->display_name ?: ($b->channel?->basic_id ?: '—') }}</td>
                                    ...
                                    <td class="px-5 py-3">
                                        <div class="flex items-center justify-end gap-1.5">
                                            @if (in_array($b->status, ['draft', 'sending'], true))
                                                <form method="POST" action="{{ route('user.line.broadcasts.start', $b->id) }}" data-bcast-start>@csrf
                                                    <button type="submit" class="px-3 py-1 rounded-full text-white text-[11px] font-semibold" style="background:#06C755">{{ $b->status === 'sending' ? __('Resume') : __('Start') }}</button>
                                                </form>
                                            @elseif ($b->status === 'failed' || ($b->failed + $b->blocked) > 0)
                                                <form method="POST" action="{{ route('user.line.broadcasts.retry', $b->id) }}">@csrf
                                            ...
                                            @endif
                                            <form method="POST" action="{{ route('user.line.broadcasts.destroy', $b->id) }}" data-confirm="{{ __('Delete this broadcast?') }}">@csrf @method('DELETE')
                            ...
                            @empty
                                <tr><td colspan="6" class="px-5 py-12 text-center text-ink-400 text-[13px]">{{ __('No broadcasts yet.') }} <a href="{{ route('user.line.broadcasts.create') }}" class="text-wa-deep font-semibold hover:underline">{{ __('Create your first one') }}</a>.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
