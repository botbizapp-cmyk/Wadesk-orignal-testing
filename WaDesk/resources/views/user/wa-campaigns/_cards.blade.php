...
@forelse ($campaigns as $c)
    @php
        ...
        // Show the greater of redirect clicks and Meta template_analytics clicks.
        $clicked = method_exists($c, 'effectiveClicks') ? $c->effectiveClicks() : (int) $c->clicked_count;
    ...
    <div class="camp-card bg-white border rounded-2xl px-5 py-[18px] transition hover:shadow-soft {{ $cardBorder }}"
        ...
        <div class="flex items-start gap-3 mb-3">
            ...
            <div class="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                ...
                {{-- Export recipient rows — same CSV export the analytics detail
                     page offers ("Export rows"), reachable straight from the card. --}}
                <a href="{{ route('user.wa-campaigns.export', $c->id) }}"
                    class="hairline border border-paper-200 rounded-full w-8 h-8 bg-paper-0 hover:bg-wa-bubble text-wa-deep flex items-center justify-center"
                    title="{{ __('Export rows') }}">
                    <svg viewBox="0 0 16 16" class="w-3.5 h-3.5" fill="none" stroke="currentColor"
