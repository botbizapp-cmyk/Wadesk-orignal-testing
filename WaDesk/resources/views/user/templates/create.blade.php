<x-layouts.user :title="__('Create Template')" nav-key="templates" page="user-templates-create">
    ...
    {{-- Instagram templates are local reusable DMs — no Meta category/language,
         no header/footer, and no "submit for review". When the channel is
         Instagram (chosen on the New-template modal) we hide those Meta-only
         sections so the form shows only what Instagram actually supports:
         name, body, media, and buttons. --}}
    @php
        $isIg = ($defaultChannel ?? '') === 'instagram';
        // Facebook mirrors Instagram: a LOCAL reusable Messenger DM (no Meta
        // approval). Both hide the Meta-only category/language/header/attachment/
        // footer sections and swap the submit label + preview chrome for the DM
        // look — $isLocalDm gates everything the two channels share.
        $isFb = ($defaultChannel ?? '') === 'facebook';
        // Telegram mirrors Instagram/Facebook: a LOCAL reusable bot message (no
        // Meta approval). Same hidden Meta-only sections + DM chrome.
        $isTg = ($defaultChannel ?? '') === 'telegram';
        $isLocalDm = $isIg || $isFb || $isTg;
        // Catalog templates are WhatsApp Cloud API (Meta official) ONLY. When the
        // format chooser opened this page with ?type=catalog we lock the whole
        // form to WABA — the send-channel picker below is filtered to WABA so the
        ...
        $bodyPlaceholder = $isTg
            ? __('Type your Telegram bot message. Press / to insert a variable like {{name}}.')
            : ($isFb
                ? __('Type your Messenger DM. Press / to insert a variable like {{name}}.')
                : ($isIg
                    ? __('Type your Instagram DM. Press / to insert a variable like {{name}}.')
                    : 'Type your message. Press / to insert a variable. Meta requires positional placeholders like {{1}} {{2}}.'));
    @endphp
        ...
        <div class="max-w-none mx-auto px-4 sm:px-6 lg:px-7 py-3 flex items-center justify-between gap-4 flex-wrap">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <button type="submit" form="templateForm"
                    ...
                    {{ $isLocalDm ? __('Save template') : __('Submit for review') }}
                </button>
            </div>
        </div>
    </div>
    ...
    <section class="max-w-none mx-auto px-4 sm:px-6 lg:px-7 py-6">
        ...
        <form id="templateForm" method="POST" action="{{ route('user.templates.store') }}"
            ...
            @if ($isCatalog && ! $catalogHasWaba)
                {{-- Catalog is WABA-only. With no WhatsApp Business number connected
                     the form can't resolve a sender, so warn up front (the server
                     also rejects a non-WABA catalog submit as defence-in-depth). --}}
                <div
                    class="xl:col-span-2 rounded-[14px] border border-accent-amber/50 bg-accent-amber/10 px-4 py-3 text-[12.5px] text-ink-800 flex items-start gap-2.5">
                    <svg viewBox="0 0 16 16" class="w-[18px] h-[18px] text-accent-amber shrink-0 mt-0.5" fill="none"
                        stroke="currentColor" stroke-width="1.6">
                        <path d="M8 2v6M8 11v.5" />
                        <circle cx="8" cy="8" r="6.5" />
                    </svg>
                    <div>
                        <div class="font-semibold">{{ __('Connect a WhatsApp Business (Meta) number first') }}</div>
                        <div class="text-[11.5px] text-ink-600 mt-0.5">
                            {{ __('Catalog templates are WhatsApp official only — they need a connected WABA number with a product catalog. Connect one, then create the catalog template.') }}
                        </div>
                    </div>
                </div>
            @endif
 ...
 available (single-engine workspaces are unchanged). --}}
                    @php
                        // Instagram is chosen via the "New template" modal's Instagram
                        // card (?channel=instagram) — it is NOT a WhatsApp send channel,
                        // so drop it from the picker on the normal (WhatsApp) create page.
                        // In IG mode we keep it (checked, section hidden) so submit still
                        // sees channel=instagram.
                        $pickerChannels = $isLocalDm
                            ? ($channels ?? [])
                            : array_values(array_filter($channels ?? [], fn ($c) => !in_array($c, ['instagram', 'facebook', 'telegram', 'sms'], true)));
                        // Catalog = WABA-only: strip every non-WABA sender so the
                        // channel picker (and therefore the resolved device/number)
                        // can only ever be a WhatsApp Business number.
                        if ($isCatalog) {
                            $pickerChannels = array_values(array_filter($pickerChannels, fn ($c) => $c === 'waba'));
                        }
                    @endphp
                    @if (count($pickerChannels) > 1)
                        {{-- When the channel was already chosen on the "New template"
                             modal (Instagram card → ?channel=instagram), the picker is
                             redundant. Keep the radios in the DOM (instagram stays
                             checked, so the field-toggling JS + submit still see the
                             channel) but hide the whole section. --}}
                        <div class="mt-3 {{ $isLocalDm ? 'hidden' : '' }}">
                            <label
                                    ...
                                    @php
                                        $cmeta =
                                            [
                                                ...
                                                'instagram' => [__('Instagram'), __('A reusable Instagram DM — ready instantly, no Meta approval.')],
                                                'facebook' => [__('Facebook'), __('A reusable Messenger DM — ready instantly, no Meta approval.')],
                                                'telegram' => [__('Telegram'), __('A reusable Telegram bot message — ready instantly, no Meta approval.')],
                                                'sms' => [__('SMS'), __('A plain-text SMS — ready instantly, no Meta approval.')],
                                            ][$ck] ?? [$ck, ''];
                                    @endphp
                                    <label
                    ...
                    @else
                        <input type="hidden" name="channel" value="{{ $pickerChannels[0] ?? 'baileys' }}">
                    @endif

                    {{-- WABA account picker (multi-WABA). When more than one WhatsApp
 Business account is connected, the operator chooses WHICH one this
 template is built on. The header/carousel media upload AND the Meta
 submit both use THIS account — using different ones makes Meta reject
 the carousel with "Bad parameter: Invalid parameter" (a media handle
 minted on WABA A is not valid on WABA B). Hidden with 0/1 WABA
 connected, where it uses the workspace's primary WABA as before. --}}
                    @if (($wabaConfigs ?? collect())->count() > 1)
                        <div class="mt-3">
                            <label
                                class="lbl text-[11.5px] font-semibold text-ink-700 flex items-center gap-2 mb-[5px]"
                                for="tpl-waba-config">
                                {{ __('WhatsApp Business account') }} <span class="req text-accent-coral">*</span>
                            </label>
                            <select id="tpl-waba-config" name="provider_config_id"
                                class="ctrl w-full px-[11px] py-[7px] border border-paper-200 rounded-lg bg-white text-[12.5px] text-ink-900 transition leading-[1.4] focus:outline-none focus:border-wa-deep focus:ring-4 focus:ring-wa-deep/10">
                                @foreach ($wabaConfigs as $wc)
                                    <option value="{{ $wc->id }}" @selected((int) old('provider_config_id', $primaryWabaId ?? 0) === (int) $wc->id)>
                                        {{ $wc->display_label ?: ('WABA #' . $wc->id) }}{{ $wc->phone_number ? ' · ' . $wc->phone_number : '' }}
                                    </option>
                                @endforeach
                            </select>
                            <div class="hint text-[10.5px] text-ink-500 mt-1 leading-[1.35]">
                                {{ __('You have more than one WABA connected — the template and its images are created on the account you pick here.') }}
                            </div>
                        </div>
                    @endif
 ...
 content is defined in Twilio's Content Template Builder (sent by SID). --}}
                ...
                <div id="standard-sections">
                    ...
                    <!-- 03 Header -->
                    <div data-catalog-hide class="sec px-[18px] py-4 hairline-b border-b border-paper-200 {{ $isLocalDm ? 'hidden' : '' }}">
                        <div class="sec-head flex items-center gap-2.5 mb-3">
                            <span
 ...
 hidden field above is kept for the legacy positional path. --}}
                            <div id="var-map-panel"
                                class="mt-2.5 rounded-lg border border-paper-200 bg-paper-50/60 px-3 py-2.5 {{ $isLocalDm ? 'hidden' : '' }}">
                    ...
                    <!-- 06 Footer -->
                    <div class="sec px-[18px] py-4 hairline-b border-b border-paper-200 {{ $isLocalDm ? 'hidden' : '' }}">
                        <div class="sec-head flex items-center gap-2.5 mb-3">
                            <span
                    ...
                    <!-- 06b Code validity (Authentication / OTP templates only) -->
                    {{-- Hidden until the operator picks the Authentication category —
                         OTP expiry is meaningless for Marketing / Utility. Shown by
                         user-templates-create.js watching #tpl-category. --}}
                    <div id="otp-section" class="sec px-[18px] py-4 hairline-b border-b border-paper-200 hidden">
                        <div class="sec-head flex items-center gap-2.5 mb-3">
                            <span
                                class="sec-num w-[23px] h-[23px] rounded-[7px] bg-paper-50 text-wa-deep inline-flex items-center justify-center text-[10px] font-semibold font-mono shrink-0">OTP</span>
                            <span
                                class="sec-title font-serif text-[18px] leading-none text-ink-900 flex-1">{{ __('Code validity') }}</span>
                            <span
                                class="sec-meta font-mono text-[10px] text-ink-500">{{ __('authentication only') }}</span>
                        </div>
                        <input id="tpl-code-expiry" name="code_expiration_minutes" type="number" min="1" max="90"
                            value="{{ old('code_expiration_minutes') }}"
                            class="ctrl w-full px-[11px] py-[7px] border border-paper-200 rounded-lg bg-white text-[12.5px] text-ink-900 transition leading-[1.4] font-sans placeholder:text-[#8A9A95] focus:outline-none focus:border-wa-deep focus:ring-4 focus:ring-wa-deep/10"
                            placeholder="10">
                        <div class="hint text-[10.5px] text-ink-500 mt-1 leading-[1.35]">
                            {{ __('For Authentication (OTP) templates only: how long the code stays valid — 1 to 90 minutes. Leave blank for 10. WhatsApp shows this to the user and uses it for one-tap autofill; your own backend still decides when a submitted code is actually expired.') }}</div>
                    </div>


                    <!-- 07 Buttons -->
                    <div data-catalog-hide class="sec px-[18px] py-4 hairline-b border-b border-paper-200">
                        <div class="sec-head flex items-center gap-2.5 mb-3">
                            <span
 ...
 processButtons() input arrays. --}}
                    ...
                    {{-- 07c Catalog (Meta official, WABA-only) — the "View catalog"
                         button + which product image is the template thumbnail.
                         Hidden by default; user-templates-create.js reveals it for
                         ?type=catalog and hides the Header / Attachment / Buttons
                         sections above (a CATALOG button can't share a template with
                         other buttons or a media header). --}}
                    <div id="catalog-sections" class="sec px-[18px] py-4 hairline-b border-b border-paper-200 hidden">
                        <div class="sec-head flex items-center gap-2.5 mb-3">
                            <span
                            ...
                            <span
                                class="sec-title font-serif text-[18px] leading-none text-ink-900 flex-1">{{ __('Catalog button') }}</span>
                            <span class="sec-meta font-mono text-[10px] text-ink-500">{{ __('WhatsApp official') }}</span>
                        </div>
                        <div
                            class="rounded-xl border border-wa-green/40 bg-wa-mint/40 px-3 py-2.5 text-[11.5px] text-ink-700 flex items-start gap-2 mb-3">
                            <svg viewBox="0 0 16 16" class="w-4 h-4 text-wa-deep shrink-0 mt-0.5" fill="none"
                                stroke="currentColor" stroke-width="1.6">
                                <path d="M3 5h10l-.7 7.2a1 1 0 0 1-1 .8H4.7a1 1 0 0 1-1-.8L3 5Z" />
                                <path d="M5.5 5V4a2.5 2.5 0 0 1 5 0v1" />
                            </svg>
                            <span>{{ __('A tappable “View catalog” button that opens your WhatsApp product catalog. Sent only from a WhatsApp Business (Meta) number.') }}</span>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label
                                    class="lbl text-[11.5px] font-semibold text-ink-700 flex items-center justify-between gap-2 mb-[5px]"
                                    for="tpl-catalog-btn">{{ __('Button label') }}</label>
                                <input id="tpl-catalog-btn" name="catalog_button_text" type="text"
                                    value="{{ old('catalog_button_text', 'View catalog') }}" maxlength="25"
                                    class="ctrl w-full px-[11px] py-[7px] border border-paper-200 rounded-lg bg-white text-[12.5px] text-ink-900 transition leading-[1.4] focus:outline-none focus:border-wa-deep focus:ring-4 focus:ring-wa-deep/10">
                                <div class="hint text-[10.5px] text-ink-500 mt-1 leading-[1.35]">
                                    {{ __('Max 25 characters. Default “View catalog”.') }}</div>
                            </div>
                            <div>
                                <label
                                    class="lbl text-[11.5px] font-semibold text-ink-700 flex items-center justify-between gap-2 mb-[5px]"
                                    for="tpl-catalog-thumb">{{ __('Thumbnail product') }} <span
                                        class="font-mono text-[10px] text-ink-500">{{ __('optional') }}</span></label>
                                @if (($catalogProducts ?? collect())->count())
                                    <select id="tpl-catalog-thumb" name="thumbnail_product_retailer_id"
                                        class="ctrl w-full px-[11px] py-[7px] border border-paper-200 rounded-lg bg-white text-[12.5px] text-ink-900 transition leading-[1.4] focus:outline-none focus:border-wa-deep focus:ring-4 focus:ring-wa-deep/10">
                                        <option value="">{{ __('Catalog cover (no specific product)') }}</option>
                                        @foreach ($catalogProducts as $p)
                                            <option value="{{ $p['retailer_id'] }}" @selected(old('thumbnail_product_retailer_id') === $p['retailer_id'])>
                                                {{ $p['name'] ?: $p['retailer_id'] }}</option>
                                        @endforeach
                                    </select>
                                @else
                                    <input id="tpl-catalog-thumb" name="thumbnail_product_retailer_id" type="text"
                                        value="{{ old('thumbnail_product_retailer_id') }}"
                                        placeholder="{{ __('Product SKU / retailer ID') }}"
                                        class="ctrl w-full px-[11px] py-[7px] border border-paper-200 rounded-lg bg-white text-[12.5px] text-ink-900 font-mono transition leading-[1.4] placeholder:text-[#8A9A95] focus:outline-none focus:border-wa-deep focus:ring-4 focus:ring-wa-deep/10">
                                @endif
                                <div class="hint text-[10.5px] text-ink-500 mt-1 leading-[1.35]">
                                    {{ __('Which product’s image shows as the template thumbnail. Must exist in your connected catalog. Leave as cover to use the catalog’s default.') }}
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <aside class="preview-col sticky top-[78px] self-start space-y-3">
                <div class="card bg-white border border-paper-200 rounded-[14px] shadow-card p-3">
                    ...
                    <div
                        ...
                        <div
                            class="phone-screen {{ $isLocalDm ? 'bg-white' : 'bg-wa-chat' }} rounded-[18px] min-h-[420px] flex flex-col overflow-hidden">
                            @if ($isFb)
                                {{-- Messenger DM header — Facebook-blue bar, back chevron,
                                     avatar, Page name + "Active now". Mirrors the Instagram
                                     preview, swapped to the Messenger look (#1877F2). --}}
                                <div class="phone-bar text-white px-3 py-2 flex items-center gap-[7px] text-[11.5px]"
                                    style="background:#1877F2">
                                    <svg viewBox="0 0 16 16" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="1.8">
                                        <path d="M10 4l-4 4 4 4" />
                                    </svg>
                                    <div class="w-6 h-6 rounded-full bg-white/25 text-white flex items-center justify-center text-[9px] font-semibold">B</div>
                                    <div class="leading-tight">
                                        <div class="text-[11.5px] font-semibold">{{ __('Bloomly') }}</div>
                                        <div class="text-[9px] opacity-80">{{ __('Active now') }}</div>
                                    </div>
                                </div>
                            @elseif ($isIg)
                                {{-- Instagram DM header — gradient bar, back chevron, avatar,
                                     @username + "Active now". Mirrors the Instaflow preview. --}}
                                <div class="phone-bar text-white px-3 py-2 flex items-center gap-[7px] text-[11.5px]"
                                    ...
                                    <div class="w-6 h-6 rounded-full bg-white/25 text-white flex items-center justify-center text-[9px] font-semibold">B</div>
                                    <div class="leading-tight">
                                        <div class="text-[11.5px] font-semibold">bloomly</div>
                                        <div class="text-[9px] opacity-80">{{ __('Active now') }}</div>
                                    </div>
                                </div>
                            @elseif ($isTg)
                                {{-- Telegram DM header — Telegram-blue bar (#229ED9), back
                                     chevron, avatar, bot name + "bot". --}}
                                <div class="phone-bar text-white px-3 py-2 flex items-center gap-[7px] text-[11.5px]"
                                    ...
                                    <div class="w-6 h-6 rounded-full bg-white/25 text-white flex items-center justify-center text-[9px] font-semibold">B</div>
                                    <div class="leading-tight">
                                        <div class="text-[11.5px] font-semibold">{{ __('Bloomly Support') }}</div>
                                        <div class="text-[9px] opacity-80">{{ __('bot') }}</div>
                                    </div>
                                </div>
                            @else
                                <div
                            ...
                            @endif
                            <div
                                class="phone-body flex-1 p-3 {{ $isLocalDm ? 'bg-white' : 'bg-wa-chat [background-image:radial-gradient(rgba(7,94,84,0.06)_1px,transparent_1px)] bg-[length:14px_14px]' }}">
                                <div class="pp-bubble {{ $isLocalDm ? 'bg-[#EFEFEF] rounded-[18px] rounded-tl-[4px]' : 'bg-paper-0 rounded-[7px] rounded-tl-[2px] shadow-[0_1px_1px_rgba(0,0,0,0.06)]' }} px-[9px] py-2 max-w-[88%] mb-[5px] text-[12px] leading-[1.4] break-words"
                                    id="pp-card">
