<x-layouts.user :title="__('Appointments')" nav-key="appointments" page="user-appointments-index">
                ...
                <div class="border border-paper-200 rounded-2xl bg-paper-0 p-4 shadow-card">
                    ...
                    @unless ($isConnected)
                        <p class="text-[11px] text-ink-500 mt-2 leading-snug">
                            {{ __('Bookings work without this. Connect only to sync free/busy, add Meet links, and mirror to your calendar.') }}
                        </p>
                        <a href="{{ url('/appointments/settings') }}"
                            class="mt-2.5 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-paper-200 bg-paper-0 hover:bg-paper-50 text-[11px] font-semibold">
                            {{ __('Connect') }}
                        </a>
                    @endunless
                </div>
                    ...
                    <div class="flex items-end justify-between gap-4 flex-wrap">
                        <div>
                            <h1 class="font-serif font-normal tracking-tight text-[32px] sm:text-[38px] lg:text-[44px] leading-none">
                                {{ __('Your') }} <span class="italic text-wa-deep">{{ __('bookings') }}</span>
                            </h1>
                            <p class="text-[13px] text-ink-600 mt-2 max-w-2xl">
                                {{ __('Let customers book appointments inside the chat. Create a booking type with your services, hours and reminders — customers pick a slot and get confirmed automatically.') }}
                            </p>
                        </div>
                        <div class="flex items-center gap-2 flex-wrap">
                            <a href="{{ url('/appointments/booking-types/create') }}"
                                ...
                                <svg viewBox="0 0 16 16" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M8 3v10M3 8h10"/></svg>
                                {{ __('New booking type') }}
                            </a>
                            <a href="{{ url('/appointments/booking-types') }}"
                                class="px-3.5 py-1.5 border border-paper-200 rounded-full bg-paper-0 hover:bg-paper-50 text-[12px] font-semibold inline-flex items-center gap-1.5">
                                <svg viewBox="0 0 16 16" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="3" width="12" height="11" rx="1.5"/><path d="M2 6h12M5 2v2M11 2v2"/></svg>
                                {{ __('Booking types') }}
                            </a>
                            <a href="{{ url('/appointments/settings') }}"
                    ...
                    <div class="bg-paper-0 border border-paper-200 rounded-2xl shadow-card overflow-hidden">
                        ...
                        @if ($upcoming->isEmpty())
                            <div class="px-5 py-12 text-center">
                                ...
                                <p class="text-[12.5px] text-ink-600 mt-1 max-w-md mx-auto">{{ __('Create a booking type with your services and hours, then drop a "Book appointment" node into any flow — confirmed bookings show up here.') }}</p>
                                <a href="{{ url('/appointments/booking-types/create') }}"
                                    class="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-wa-deep hover:bg-wa-teal text-paper-0 text-[12px] font-semibold">
                                    <svg viewBox="0 0 16 16" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M8 3v10M3 8h10"/></svg>
                                    {{ __('New booking type') }}
                                </a>
                            </div>
