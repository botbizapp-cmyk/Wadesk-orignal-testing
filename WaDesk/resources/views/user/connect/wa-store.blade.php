        ...
        @elseif ($mode === 'list')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                @foreach ($shops as $shop)
                    @php
                        // Resolves BOTH engines: a Baileys device_id OR (device_id null)
                        // the workspace's connected official WABA/Twilio number — so an
                        // official store no longer shows "No device". See WaStorefront.
                        $devicePhone = $shop->sending_label;
                    @endphp
                    <div
                        ...
                        @php $shopBrand = ($shop->settings_json['brand_color'] ?? '') ?: 'var(--color-wa-deep)'; @endphp
                        <div class="h-20 bg-gradient-to-br relative"
                            style="background: linear-gradient(135deg, {{ $shopBrand }}, color-mix(in srgb, {{ $shopBrand }} 80%, transparent))">
                            <span
                ...
                <section class="p-6 border-b border-paper-200">
                    ...
                    @else
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {{-- Official API (Meta / WABA) — a connected WABA channel is a
                                 valid sender too. The store SEND routes via WorkspaceEngine::for()
                                 (WABA/Twilio → provider config), so it needs no Baileys device_id;
                                 selecting this stores device_id = null and the workspace engine
                                 sends. Without this a WABA-only account saw "no devices" and was
                                 blocked at step 3 even with a live WABA number. --}}
                            @if ($hasWaba)
                                @php
                                    $wabaPhone   = $providerConfig?->phone_number ?: '';
                                    $wabaLabel   = $providerConfig?->display_label ?: __('Official API · Meta (WABA)');
                                    // Default to WABA when there is no bound device and no Baileys device.
                                    $wabaChecked = empty(old('device_id', $defaultDeviceId));
                                @endphp
                                <label class="cursor-pointer block">
                                    <input type="radio" name="device_id" value="" @checked($wabaChecked) class="sr-only peer" />
                                    <div
                                        ...
                                        <span class="w-9 h-9 rounded-full bg-wa-bubble/70 grid place-items-center shrink-0">
                                            <svg viewBox="0 0 24 24" class="w-4 h-4 text-wa-deep" fill="none" stroke="currentColor" stroke-width="1.7">
                                                <path d="M12 21a9 9 0 1 0-8.5-6" /><path d="M3 21l1.5-4.5" />
                                            </svg>
                                        </span>
                                        <div class="min-w-0">
                                            <div class="font-semibold text-[12.5px] truncate">{{ $wabaLabel }}</div>
                                            <div class="font-mono text-[11px] text-ink-500 truncate">
                                                {{ $wabaPhone ?: __('Workspace WhatsApp Business number') }}</div>
                                        </div>
                                    </div>
                                </label>
                            @endif
