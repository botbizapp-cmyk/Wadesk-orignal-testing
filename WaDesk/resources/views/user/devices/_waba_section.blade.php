...
<section class="space-y-5">
    ...
    @if ($wabaAccounts->isNotEmpty())
        @php
            $bspWs        = auth()->user()?->currentWorkspace;
            $bspReseller  = (bool) ($bspWs?->billsToPlatform());
            $bspAvailable = class_exists(\App\Bsp\Services\CreditAllocationService::class)
                && app(\App\Bsp\Services\CreditAllocationService::class)->source()->isConfigured();
        @endphp
        <div class="bg-paper-0 border border-paper-200 rounded-2xl p-5 shadow-card">
            <div class="font-mono text-[10px] uppercase tracking-[0.16em] text-ink-500">{{ __('billing for this number') }}</div>
            <h3 class="font-serif text-[18px] leading-tight mt-0.5 mb-3">{{ __('How do you want to be billed for messages?') }}</h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {{-- Card A — Meta bills directly (normal system) --}}
                <form method="POST" action="{{ route('user.devices.billing-mode') }}">
                    @csrf
                    <input type="hidden" name="mode" value="meta_direct">
                    <button type="submit"
                        class="w-full text-left rounded-xl border-2 p-4 transition {{ !$bspReseller ? 'border-wa-deep bg-wa-deep/5' : 'border-paper-200 hover:border-wa-deep/40' }}">
                        <div class="flex items-center justify-between gap-2">
                            <span class="font-serif text-[15px] text-ink-900">{{ __('I pay Meta directly') }}</span>
                            @if (!$bspReseller)<span class="text-[10px] font-mono px-2 py-0.5 rounded-full bg-wa-deep text-paper-0">{{ __('Active') }}</span>@endif
                        </div>
                        <p class="text-[12px] text-ink-600 mt-1.5 leading-relaxed">
                            {{ __('Your own payment method is on Meta — Meta bills you for messages. You just need an active plan for features.') }}
                        </p>
                    </button>
                </form>
                {{-- Card B — Bill through us (BSP wallet) --}}
                <form method="POST" action="{{ route('user.devices.billing-mode') }}" @if(!$bspAvailable) onsubmit="return false" @endif>
                    @csrf
                    <input type="hidden" name="mode" value="reseller">
                    <button type="submit" @disabled(!$bspAvailable)
                        class="w-full text-left rounded-xl border-2 p-4 transition disabled:opacity-50 disabled:cursor-not-allowed {{ $bspReseller ? 'border-wa-deep bg-wa-deep/5' : 'border-paper-200 hover:border-wa-deep/40' }}">
                        <div class="flex items-center justify-between gap-2">
                            <span class="font-serif text-[15px] text-ink-900">{{ __('Bill me through :brand', ['brand' => brand_name()]) }}</span>
                            @if ($bspReseller)<span class="text-[10px] font-mono px-2 py-0.5 rounded-full bg-wa-deep text-paper-0">{{ __('Active') }}</span>
                            @elseif (!$bspAvailable)<span class="text-[10px] font-mono px-2 py-0.5 rounded-full bg-paper-100 text-ink-500">{{ __('Not available yet') }}</span>@endif
                        </div>
                        <p class="text-[12px] text-ink-600 mt-1.5 leading-relaxed">
                            {{ __('Top up a wallet here — each message is charged from it at our rate. No Meta payment method needed; we handle Meta billing.') }}
                        </p>
                    </button>
                </form>
            </div>
                    ...
                    <div
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                @endif
                                {{-- Register on WhatsApp Cloud API — completes registration so
                                     Meta stops rejecting sends with (#133010) "Account not
                                     registered". Shown regardless of CONNECT state (a PENDING pure
                                     Cloud-API number needs exactly this), but ONLY for
                                     non-coexistence numbers: Meta rejects /register for SMB /
                                     coexistence numbers with "(#100) Register endpoint is not
                                     available for SMB businesses", so the button would only ever
                                     fail there (they don't use Cloud-API registration anyway). --}}
                                @unless ($isCoexistence)
                                    <form method="POST" action="{{ url('/devices/waba/' . $waba->id . '/register') }}"
                                        class="inline"
                                        data-prompt-reason="Registers {{ $verified }} on the WhatsApp Cloud API so Meta stops rejecting sends (#133010 “Account not registered”). If this number has a two-step-verification PIN, enter it below — otherwise leave blank."
                                        data-prompt-title="Register on Cloud API"
                                        data-prompt-placeholder="Two-step PIN (optional)"
                                        data-prompt-field="pin"
                                        data-confirm-text="Register">
                                        @csrf
