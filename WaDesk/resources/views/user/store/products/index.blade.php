<x-layouts.user :title="__('Products')" nav-key="connect" page="user-store-products-index">
    ...
    <main class="max-w-none mx-auto px-4 sm:px-6 lg:px-7 py-7">
        <div class="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
            ...
            <section class="space-y-5 min-w-0">
                ...
                {{-- Card is a plain <div>, NOT a <form> — the per-row Delete forms
                     live inside this card, and nesting them in a wrapping <form>
                     is invalid HTML (the browser drops the inner form, so clicking
                     Delete just re-submitted the search). The search box is now its
                     OWN small form so it still filters, and Delete works. --}}
                <div class="bg-paper-0 border border-paper-200 rounded-[14px] shadow-card">
                    <div class="px-4 py-3 border-b border-paper-200">
                        <form method="GET" class="relative max-w-[480px]">
    ...
    <div id="share-modal" class="hidden fixed inset-0 z-50 bg-ink-900/40 grid place-items-center px-4"
        ...
        <div class="bg-paper-0 rounded-2xl shadow-soft max-w-md w-full p-5">
            ...
            <div class="mt-4 space-y-2.5 text-[12.5px]">
                ...
                {{-- Send the product straight to a customer's WhatsApp (image +
                     details) from your connected device — no need to copy a link. --}}
                <div class="pt-2.5 mt-1 border-t border-paper-200">
                    <div class="font-semibold mb-1">{{ __('Send to a customer') }}</div>
                    <div class="flex items-center gap-2">
                        <input id="share-send-phone" type="tel" inputmode="tel"
                            placeholder="{{ __('WhatsApp number, e.g. +1 555…') }}"
                            class="flex-1 px-2.5 py-1.5 border border-paper-200 rounded-lg bg-paper-0 text-[11.5px]" />
                        <button type="button" id="share-send-btn" onclick="sendProductToNumber(this)"
                            class="px-3 py-1.5 rounded-full bg-wa-deep hover:bg-wa-teal text-paper-0 text-[11.5px] font-semibold shrink-0">{{ __('Send') }}</button>
                    </div>
                    <div id="share-send-msg" class="text-[11px] mt-1.5 hidden"></div>
                </div>
            </div>
    ...
    <script>
        ...
        async function shareProduct(id) {
            ...
            // reset the send-to-customer row each open
            const phone = document.getElementById('share-send-phone');
            const msg = document.getElementById('share-send-msg');
            if (phone) phone.value = '';
            if (msg) { msg.className = 'text-[11px] mt-1.5 hidden'; msg.textContent = ''; }
            document.getElementById('share-modal').classList.remove('hidden');
        }

        async function sendProductToNumber(btn) {
            const phone = (document.getElementById('share-send-phone').value || '').trim();
            const msg = document.getElementById('share-send-msg');
            const show = (text, ok) => {
                msg.textContent = text;
                msg.className = 'text-[11px] mt-1.5 ' + (ok ? 'text-wa-deep' : 'text-accent-coral');
            };
            if (!currentShareProductId) return;
            if (phone.replace(/\D/g, '').length < 7) return show('Enter a valid WhatsApp number with country code.', false);
            btn.disabled = true;
            const original = btn.textContent;
            btn.textContent = 'Sending…';
            try {
                const res = await fetch(`/store/products/${currentShareProductId}/send`, {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json',
                ...
                show(j.message || (res.ok && j.ok ? 'Sent.' : 'Could not send.'), !!(res.ok && j.ok));
                if (res.ok && j.ok) document.getElementById('share-send-phone').value = '';
            } catch (e) {
                show('Send failed: ' + e.message, false);
            } finally {
                btn.disabled = false;
