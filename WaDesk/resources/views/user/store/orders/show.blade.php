<x-layouts.user :title="__('Order') . ' #' . $order->id" nav-key="connect" page="user-store-orders-show">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
                    ...
                    <div class="space-y-5 min-w-0">
                        ...
                        <form method="POST" action="{{ route('user.store.orders.update', $order->id) }}"
                            ...
                            {{-- Native in-chat checkout — hosted pay link on the merchant's own
                                 gateway (any of 32). Engine-agnostic (WABA + Unofficial). --}}
                            @if (!empty($checkoutGateways ?? []))
                                <div class="border-t border-paper-200 pt-3">
                                    <span class="text-[11.5px] font-semibold text-ink-700">{{ __('Send checkout (hosted pay link)') }}</span>
                                    <div class="flex items-center gap-2 mt-1 flex-wrap">
                                        <select id="co-gateway" class="px-3 py-2 border border-paper-200 rounded-lg text-[13px] bg-paper-0 focus:outline-none focus:border-wa-deep">
                                            @foreach ($checkoutGateways as $g)
                                                <option value="{{ $g }}">{{ $gatewayNames[$g]['name'] ?? ucfirst($g) }}</option>
                                            @endforeach
                                        </select>
                                        <button type="button" onclick="sendCheckout()"
                                            class="px-3 py-1.5 border border-wa-deep/40 text-wa-deep rounded-full text-[12px] hover:bg-wa-mint/40 font-semibold whitespace-nowrap">{{ __('Send checkout link') }}</button>
                                    </div>
                                    <span class="text-[10.5px] text-ink-500 mt-0.5 block">{{ __('Charges into your own gateway account. Works on WhatsApp API + Unofficial numbers.') }}</span>
                                </div>
                            @else
                                <div class="border-t border-paper-200 pt-3 text-[11.5px] text-ink-500">
                                    {{ __('No checkout gateway configured yet.') }}
                                    <a href="{{ route('user.store.gateways.index') }}" class="text-wa-deep font-semibold hover:underline">{{ __('Add gateway keys →') }}</a>
                                </div>
                            @endif
    ...
    <script>
        ...
        async function sendCheckout() {
            const sel = document.getElementById('co-gateway');
            const gateway = sel ? sel.value : '';
            if (!gateway) { alert('Pick a gateway.'); return; }
            const r = await fetch(@json(route('user.store.orders.send-checkout', $order->id)), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
            ...
            const j = await r.json().catch(() => ({}));
            if (j.ok) {
                alert('Checkout link sent to the customer.');
                location.reload();
            } else {
                alert(j.message || 'Could not send checkout link.');
            }
        }
    </script>
</x-layouts.user>
