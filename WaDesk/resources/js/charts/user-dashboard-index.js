...
export default function init() {
    ...
    if (spark) {
        new ApexCharts(spark, {
            ...
            plotOptions: { bar: {
                ...
                colors: { ranges: [
                    { from: 0,  to: 9,    color: themeColor('wa-teal') },
                    { from: 10, to: 9999, color: themeColor('wa-green') },
                ] },
            } },
    ...
    if (radial) {
        new ApexCharts(radial, {
            ...
            plotOptions: { radialBar: {
                ...
                dataLabels: {
                    name: { show: false },
                    value: { show: true, fontSize: "10px", fontFamily: "JetBrains Mono, monospace", fontWeight: 600, color: themeColor('wa-deep'), offsetY: 4, formatter: (v) => Number(v).toFixed(1) },
                },
            } },
    ...
    if (tp) {
        ...
        // Default to the window the SERVER rendered the KPI cards for (from
        // ?range=), so the chart and the cards always agree on first paint.
        const activeRange = d.activeRange || "7d";
        const initial = getRange(activeRange);
        const throughputChart = new ApexCharts(tp, {
            ...
            xaxis: {
                ...
                labels: { rotate: -45, rotateAlways: false, hideOverlappingLabels: true, trim: false, style: { fontFamily: "JetBrains Mono, monospace", fontSize: "10px", colors: themeColor('ink-500') } },
                tickPlacement: "on",
        ...
        // Switching the range RELOADS with ?range=<key> so the whole header
        // (Messages-sent, Read-rate, Active-contacts + their trend arrows) is
        // recomputed server-side for that window — not just the chart. Reloading
        // is what fixes "changing the date doesn't update the cards".
        ...
        // Reflect the server's active window on first paint (highlight the
        // matching button + set the chart subtitle; the chart itself already
        // rendered `activeRange` above).
        setActive(document.querySelector(`[data-range="${activeRange}"]`));
        if (subtitleEl && subtitles[activeRange]) subtitleEl.textContent = subtitles[activeRange];
