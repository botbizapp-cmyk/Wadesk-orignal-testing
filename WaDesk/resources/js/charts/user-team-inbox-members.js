...
export default function init() {
    ...
    function render() {
        ...
        tbody.innerHTML = filtered.map(m => {
            ...
            const resetBtn = canRemove
                ? `<button data-reset="${m.id}" data-name="${escape(m.name)}" class="w-8 h-8 rounded-full hover:bg-wa-mint/50 text-wa-deep grid place-items-center" title="Reset password">
                     <svg viewBox="0 0 16 16" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M11 6V4.5a3 3 0 0 0-6 0V6M3.5 6h9v7h-9zM8 9v2"/></svg>
                   </button>` : '';
            ...
            return `<tr>
              ...
              <td class="pl-3 pr-5 py-3 text-right"><div class="inline-flex items-center gap-1">${resetBtn}${removeBtn}</div></td>
            </tr>`;
        }).join('');
        ...
        // wire reset-password buttons — generate a fresh temp password and show
        // it ONCE (the reliable hand-off for existing accounts / dead mail).
        $$('button[data-reset]').forEach(btn => {
            btn.addEventListener('click', async () => {
                const id = parseInt(btn.dataset.reset, 10);
                if (!confirm(`Reset ${btn.dataset.name}'s password? A new temporary password will be shown once — their current password stops working immediately.`)) return;
                btn.disabled = true;
                try {
                    const data = await api(`/team-inbox/api/members/${id}/reset-password`, { method: 'POST' });
                    showCredsBox(data);
    ...
    // Render the new credentials in a dismissible banner above the members
    // table so the admin can copy them. Shown once — never re-fetchable.
    function showCredsBox(data) {
        if (!data || !data.temp_password) return;
        let box = $('#member-cred-box');
        if (!box) {
            box = document.createElement('div');
            box.id = 'member-cred-box';
            box.className = 'mb-4 rounded-xl border border-wa-deep/20 bg-wa-mint/40 text-wa-deep px-4 py-3';
            const anchor = $('#members-tbody')?.closest('.overflow-x-auto, section, div');
            (anchor?.parentElement || document.body).insertBefore(box, anchor || null);
        }
        const tail = data.email_sent
            ? 'We also emailed it to them. Share this as a backup if the email doesn’t arrive:'
            : 'Email isn’t configured on this server, so share these credentials with them directly:';
        box.innerHTML = `
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <div class="font-semibold mb-0.5">New password for ${escape(data.member.name)}</div>
              <div class="text-[11.5px]">${tail}</div>
              <div class="font-mono text-[11.5px] bg-paper-0 border border-paper-200 rounded px-2 py-1.5 mt-1.5 select-all break-all">
                Email: ${escape(data.member.email)}<br/>
                Password: ${escape(data.temp_password)}
              </div>
            </div>
            <button type="button" data-close-cred class="shrink-0 text-ink-500 hover:text-ink-900" title="Dismiss">
              <svg viewBox="0 0 16 16" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M4 4l8 8M12 4l-8 8"/></svg>
            </button>
          </div>`;
        box.querySelector('[data-close-cred]')?.addEventListener('click', () => box.remove());
    ...
    $('#invite-form')?.addEventListener('submit', async e => {
        ...
        try {
            ...
            if (data.temp_password) {
                // Brand-new account — ALWAYS show the credentials box (with the
                // temp password) as the reliable backup, whether or not email
                // actually went out. Email delivery on self-hosted installs is
                // unreliable, so this box is the operator's dependable hand-off.
                const tail = data.email_sent
                    ? 'Email sent. As a backup, the temp password is below — share it if the email doesn’t arrive:'
                    : 'Share these sign-in credentials with them manually:';
                result.innerHTML = `
                ...
                // Email already had a WaDesk account — no new password exists to
                // show. Render a clear note (consistent box, never a blank
