...
export default function init() {
    ...
    function refreshPreview() {
        if (!tplSel) return; // Instagram has no template picker — its own code owns the preview.
    ...
    function applyScheduleMode(mode) {
        ...
        // Instagram hides the schedule section, so these can be absent — guard them.
        if (mode === 'later') {
            schedFields?.classList.remove('hidden'); schedFields?.classList.add('grid');
            if (schedLabel) schedLabel.textContent = 'Later';
        } else {
            schedFields?.classList.add('hidden'); schedFields?.classList.remove('grid');
            if (schedLabel) schedLabel.textContent = 'Now';
        }
    }
    ...
    function updateSelectedCount() {
        const n = $$('[data-contact]:checked').length;
        if (selCount) selCount.textContent = String(n);
        if (selSum)   selSum.textContent   = n + ' selected';
    }
    tbody?.addEventListener('change', (e) => {
        if (e.target.matches('[data-contact]')) updateSelectedCount();
    });
    selectAll?.addEventListener('change', (e) => {
        $$('[data-contact]').forEach(cb => {
            if (cb.closest('tr')?.classList.contains('hidden')) return;
            cb.checked = e.target.checked;
        });
        updateSelectedCount();
    });
    // Combined row filter — search text + (Instagram) account. IG rows carry a
    // data-ig-account so picking a "Send from" account narrows the chat list.
    const igAccount = $('#igAccount');
    function applyRowFilter() {
        if (!tbody) return;
        const q = (search?.value || '').trim().toLowerCase();
        const acct = igAccount ? igAccount.value : null;
        tbody.querySelectorAll('tr[data-search]').forEach(row => {
            const okSearch = q === '' || (row.dataset.search || '').includes(q);
            const okAcct = !acct || !row.dataset.igAccount || row.dataset.igAccount === acct;
            const hide = !(okSearch && okAcct);
            row.classList.toggle('hidden', hide);
            if (hide) { const cb = row.querySelector('[data-contact]'); if (cb) cb.checked = false; }
        });
        updateSelectedCount();
    }
    search?.addEventListener('input', applyRowFilter);
    igAccount?.addEventListener('change', applyRowFilter);

    // Instagram: template select prefills the message; message drives the preview.
    // Instagram: "Write text" / "Use template" toggle.
    const igTypeBtns = $$('.igtype-btn');
    const igTplWrap = $('#igTemplateWrap');
    if (igTypeBtns.length) {
        const setType = (type) => {
            igTypeBtns.forEach(b => {
                const on = b.dataset.igtype === type;
                b.classList.toggle('bg-white', on);
                b.classList.toggle('text-wa-deep', on);
                b.classList.toggle('shadow-sm', on);
                b.classList.toggle('text-ink-600', !on);
            });
            if (igTplWrap) igTplWrap.classList.toggle('hidden', type !== 'template');
        };
        igTypeBtns.forEach(b => b.addEventListener('click', () => setType(b.dataset.igtype)));
        setType('text');
    }

    const igTpl = $('#igTemplate');
    const igMsg = $('#igMessage');
    const prevBody = $('#previewBody');
    if (igTpl && igMsg) {
        let TPL = [];
        try { TPL = JSON.parse(igTpl.dataset.igTemplates || '[]') || []; } catch (e) { TPL = []; }
        const byId = {}; TPL.forEach(t => { byId[String(t.id)] = t; });
        igTpl.addEventListener('change', () => {
            const t = byId[igTpl.value];
            if (t) { igMsg.value = t.body || ''; igMsg.dispatchEvent(new Event('input')); }
        });
    }
    if (igMsg && prevBody) {
        const paintIg = () => {
            const txt = (igMsg.value || '').replace(/\{name\}/g, 'there').trim();
            prevBody.textContent = txt || 'Your message preview…';
        };
        igMsg.addEventListener('input', paintIg);
        paintIg();
    }
