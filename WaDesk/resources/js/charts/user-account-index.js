...
export default function init() {
    ...
    // ---- tom-select on timezone ----
    // The <select> is server-rendered with the CANONICAL IANA list
    // (\DateTimeZone::listIdentifiers()) — exactly like the Workspace settings
    // page. We deliberately do NOT repopulate it from Intl.supportedValuesOf()
    // here: some browsers' ICU still lists the legacy alias "Asia/Calcutta",
    // which Carbon rejects (InvalidTimeZoneException) and 500'd the dashboard
    // the moment it was saved. Just upgrade the existing options to a
    // searchable combobox — the option list stays canonical.
