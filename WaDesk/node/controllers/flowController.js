...
export const startFlow = async (req, res, app) => {
  ...
  (async () => {
    try {
      ...
      // Fix #3: normalize BOTH halves to digits so this fallback-start key
      // MATCHES the one the inbound resume path builds. providerFlowController
      // keys resume as `${digits(deviceNumber)}_${digits(customerPhone)}`; this
      // path used raw `${senderPhoneNumber}_${targetPhoneNumber}` (with `+`/
      // spaces from the contact's stored country_code+mobile), so a flow started
      // here could NEVER be resumed — it sent the first node then stalled at the
      // first question forever. Digit-normalizing both ends aligns the keys.
      const sessionKey = `${String(senderPhoneNumber).replace(/\D+/g, "")}_${String(targetPhoneNumber).replace(/\D+/g, "")}`;
