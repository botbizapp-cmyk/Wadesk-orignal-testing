// controllers/viberFlowController.js
// =================================
// Viber (REST Bot API) inbound → Node flow engine. Same shape as
...
// and run the flow detached (Delay = real await). Sends go directly to Viber (the
// token is static). PURELY ADDITIVE. Key = the Viber user id.
import { runFlow, resumeFlow, canResume, hasSession, pruneSessions } from "../services/viberFlowService.js";

/**
 * POST /api/viber-flow/inbound
 *   channelId, workspaceId, userId, text, auth({base, token, sender}), flow?, flowId?, vars?
 * Auth: X-Node-Token. Response: { ok, consumed, mode }
 */
export const viberInbound = async (req, res) => {
  ...
  console.log(`[VB-FLOW-NODE] IN channel=${channelId} user=${userId} text="${text.slice(0, 50)}" restart=${restart} isResume=${isResume} canStart=${canStart}`);
  ...
  (async () => {
    try {
      ...
      if (!done) console.log(`[VB-FLOW-NODE] resume declined (no matching branch) channel=${channelId} user=${userId}`);
    } catch (e) {
      console.error(`[VB-FLOW-NODE] handler crashed channel=${channelId} user=${userId}: ${e?.message}`);
    }
  })();
};

/** GET /api/viber-flow/health */
export const viberFlowHealth = (req, res) => {
  const expected = process.env.NODE_WEBHOOK_TOKEN || "";
  if (!expected || (req.headers["x-node-token"] || "") !== expected) {
    return res.status(401).send({ ok: false });
  }
  return res.status(200).send({ ok: true, service: "viber-flow" });
};

export default { viberInbound, viberFlowHealth };
