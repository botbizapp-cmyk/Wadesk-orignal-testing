...
function replacePlaceholders(text, attributes, variableMap) {
  ...
  if (attributes && typeof attributes === 'object') {
    ...
    // Tolerant pass — resolve DISPLAY tokens the exact-key loop above misses:
    // `{{First Name}}` / `{{first name}}` / `{{First-Name}}` all normalize to the
    // `first_name` attribute key. Without this a spaced/capitalized token shipped
    // to the customer literally on scheduled sends.
    newText = newText.replace(/\{\{\s*([\w.\- ]+?)\s*\}\}/g, (match, rawKey) => {
      const key = String(rawKey).trim();
      const candidates = [key, key.toLowerCase(), key.toLowerCase().replace(/[\s-]+/g, '_')];
      for (const k of candidates) {
        if (attributes[k] !== undefined && attributes[k] !== null && attributes[k] !== '') return attributes[k];
      }
      return match;
    });
  }
