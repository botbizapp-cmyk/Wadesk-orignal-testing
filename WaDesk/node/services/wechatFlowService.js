// services/wechatFlowService.js
// ============================
// WeChat Official Account flow engine — the Node counterpart of
// lineFlowService / telegramFlowService. WeChat's SHARED access_token is managed
// in PHP, so unlike the other engines this one holds NO WeChat token and
// delegates every SEND back to Laravel (/api/wechat/flow-send). Node only
// orchestrates: walk the graph, real `await` on Delay, park/resume, and ask PHP
// for smart nodes (ai/webhook via /api/wechat/flow-node).
//
// `buttons` render as a WeChat msgmenu (a tapped item comes back as a text
// message, which resume matches by label or 1-based number). PURELY ADDITIVE.
import axios from "axios";

const WC_SESSIONS = new Map(); // `${channelId}_${openid}` → session

const nodeHeaders = () => ({ "X-Node-Token": process.env.NODE_WEBHOOK_TOKEN || "", Accept: "application/json" });
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
const sessionKeyFor = (channelId, openid) => `${channelId}_${openid}`;

// ── Send layer — everything goes through PHP (managed token + inbox mirror) ───
async function phpSend(appDomain, payload) {
  try {
    await axios.post(`${appDomain}/api/wechat/flow-send`, payload, { headers: nodeHeaders(), timeout: 20000, validateStatus: () => true });
  } catch (e) {
    console.warn(`[WC-FLOW-NODE] flow-send failed: ${e?.message}`);
  }
}
const sendText   = (ctx, text)        => phpSend(ctx.appDomain, { channelId: ctx.channelId, openid: ctx.openid, kind: "text", text: String(text || "") });
const sendChoices = (ctx, text, opts) => phpSend(ctx.appDomain, { channelId: ctx.channelId, openid: ctx.openid, kind: "menu", text: String(text || ""), options: opts.map((o) => ({ title: String(o.title ?? o) })) });
const sendMedia  = (ctx, kind, url, caption) => phpSend(ctx.appDomain, { channelId: ctx.channelId, openid: ctx.openid, kind: "media", mediaKind: kind, mediaUrl: url, text: caption || "" });

async function askLaravel(appDomain, payload) {
  const r = await axios.post(`${appDomain}/api/wechat/flow-node`, payload, { headers: nodeHeaders(), timeout: 60000, validateStatus: () => true });
  if (r.status >= 400) throw new Error(`flow-node HTTP ${r.status}: ${JSON.stringify(r.data)}`);
  return r.data || {};
}

// ── Graph helpers (identical to the LINE/Telegram engines) ───────────────────
...
function evalOneCondition(c, vars) {
  ...
  switch (op) {
    ...
    default:
      console.warn(`[WC-NODE] condition: unknown operator "${c?.operator}" — treated as FALSE`);
      return false;
  }
}
function evalCondition(d, vars) {
  ...
  console.log(`[WC-NODE] condition rules=${rules.length} -> ${met ? "TRUE (yes port)" : "FALSE (no port)"}`);
  return met;
}

// ── The walker ───────────────────────────────────────────────────────────────
async function walk(ctx, startId) {
  const { flow, openid, appDomain, channelId, flowId, workspaceId } = ctx;
  const nodes = indexNodes(flow);
  let current = startId;
  let guard = 0;
  console.log(`[WC-WALK] start flow=${flowId} openid=${openid} from=${startId} nodes=${nodes.size}`);

  while (current && guard++ < 100) {
    const node = nodes.get(String(current));
    if (!node) { console.warn(`[WC-WALK] node id="${current}" NOT FOUND — ending flow=${flowId}`); break; }
    const type = String(node.type || "");
    const d = node.data || {};
    let port = "out";
    console.log(`[WC-NODE] → type=${type} id=${node.id} flow=${flowId}`);
    ...
    try {
      switch (type) {
        ...
        case "ai":
        case "wechat_ai": {
          const out = await askLaravel(appDomain, { action: "ai", node: d, vars: ctx.vars, workspaceId, channelId, openid });
        ...
        case "end":
          clearSession(channelId, openid);
          console.log(`[WC-FLOW-NODE] end flow=${flowId} openid=${openid}`);
          return;
        default:
  ...
  if (guard >= 100) console.warn(`[WC-FLOW-NODE] walk hit the 100-node guard (flow=${flowId})`);
  clearSession(channelId, openid);
}

// ── Session state ────────────────────────────────────────────────────────────
function park(ctx, nodeId) {
  const key = sessionKeyFor(ctx.channelId, ctx.openid);
  WC_SESSIONS.set(key, {
    channelId: ctx.channelId, openid: ctx.openid, workspaceId: ctx.workspaceId,
    flowId: ctx.flowId, flow: ctx.flow, appDomain: ctx.appDomain,
    nodeId: String(nodeId), vars: ctx.vars, parkedAt: Date.now(),
  });
  console.log(`[WC-FLOW-NODE] parked at node=${nodeId} key=${key}`);
}
function clearSession(channelId, openid) { WC_SESSIONS.delete(sessionKeyFor(channelId, openid)); }
export const hasSession = (channelId, openid) => WC_SESSIONS.has(sessionKeyFor(channelId, openid));
export function pruneSessions(maxAgeMs = 86_400_000) {
  const cutoff = Date.now() - maxAgeMs; let n = 0;
  for (const [k, s] of WC_SESSIONS) if (s.parkedAt < cutoff) { WC_SESSIONS.delete(k); n++; }
  return n;
}

// ── Public API ───────────────────────────────────────────────────────────────
export async function runFlow({ flow, openid, text, flowId, channelId, workspaceId, appDomain, vars }) {
  clearSession(channelId, openid);
  const start = entryNode(flow);
  if (!start) { console.warn(`[WC-FLOW-NODE] flow ${flowId} has no trigger node`); return false; }
  const ctx = {
    flow, openid, flowId, channelId, workspaceId, appDomain,
    vars: { text: String(text || ""), openid: String(openid), ...(vars || {}) },
  };
  console.log(`[WC-FLOW-NODE] START flow=${flowId} channel=${channelId} openid=${openid}`);
 ...
 */
export function canResume(channelId, openid, text) {
  const sess = WC_SESSIONS.get(sessionKeyFor(channelId, openid));
...
export async function resumeFlow({ channelId, openid, text, vars }) {
  const key = sessionKeyFor(channelId, openid);
  const sess = WC_SESSIONS.get(key);
  if (!sess) { console.log(`[WC-RESUME] no session key=${key}`); return false; }
  ...
  if (!parked) { console.warn(`[WC-RESUME] parked node "${sess.nodeId}" missing — dropping`); WC_SESSIONS.delete(key); return false; }

  const d = parked.data || {};
  const type = String(parked.type || "");
  const t = String(text || "").toLowerCase().trim();
  let port = "out";
  console.log(`[WC-RESUME] key=${key} parkedNode=${sess.nodeId} type=${type} reply="${t.slice(0, 40)}"`);
  ...
  if (type === "buttons") {
    ...
    console.log(`[WC-RESUME] button match reply="${t}" → idx=${idx}`);
