// services/viberFlowService.js
// ===========================
// Viber (REST Bot API) flow engine — the Node counterpart of lineFlowService.
// Viber's auth token is STATIC, so (like LINE, unlike WeChat) this engine sends
// DIRECTLY to Viber and only calls Laravel to mirror messages (flow-log) and
// resolve AI / webhook nodes (flow-node).
//
// `buttons` render as a Viber inline keyboard: a `reply` button's ActionBody comes
// back as an ordinary message, which resume matches (by label or 1-based number).
// PURELY ADDITIVE. Recipient key = the Viber user id.
import axios from "axios";

const VB_SESSIONS = new Map(); // `${channelId}_${userId}` → session
...
// ── Viber send layer ─────────────────────────────────────────────────────────
async function viberCall(auth, path, body) {
  const base = String(auth?.base || "https://chatapi.viber.com/pa").replace(/\/+$/, "");
  const token = String(auth?.token || "");
  if (!token) throw new Error("viber auth incomplete (token)");
  ...
  const data = r.data || {};
  if (data.status !== 0) throw new Error(data.status_message || `viber ${path} status ${data.status}`);
  return data;
}
const midOf = (res) => (res && res.message_token) || null;
const senderOf = (auth) => auth?.sender || { name: "Bot" };

const sendText = (auth, userId, text) =>
  viberCall(auth, "/send_message", { receiver: userId, min_api_version: 1, sender: senderOf(auth), type: "text", text: String(text || "") });

const sendChoices = (auth, userId, text, opts) => {
  const buttons = opts
    .filter((o) => String(o.title ?? o).trim() !== "")
    .slice(0, 12)
    .map((o) => ({ Columns: 6, Rows: 1, ActionType: "reply", ActionBody: String(o.title ?? o), Text: String(o.title ?? o), TextSize: "regular", BgColor: "#7360F2" }));
  const body = { receiver: userId, min_api_version: 3, sender: senderOf(auth), type: "text", text: String(text || "") };
  if (buttons.length) body.keyboard = { Type: "keyboard", DefaultHeight: false, Buttons: buttons };
  return viberCall(auth, "/send_message", body);
};

function mediaBody(auth, userId, kind, url, caption) {
  const base = { receiver: userId, min_api_version: 1, sender: senderOf(auth) };
  switch (String(kind || "").toLowerCase()) {
    case "video": return { ...base, type: "video", media: url, size: 1 };
    case "audio": case "voice": case "file": case "document":
      return { ...base, type: "file", media: url, size: 1, file_name: url.split("/").pop() || "file" };
    default: return { ...base, type: "picture", media: url, text: caption || "" };
  }
}
const sendMedia = (auth, userId, kind, url, caption) => viberCall(auth, "/send_message", mediaBody(auth, userId, kind, url, caption));

// ── Laravel bridges (mirror + AI/webhook stay in PHP) ────────────────────────
async function logToLaravel(appDomain, payload) {
  try {
    await axios.post(`${appDomain}/api/viber/flow-log`, payload, { headers: nodeHeaders(), timeout: 15000 });
  } catch (e) {
    console.warn(`[VB-FLOW-NODE] flow-log failed: ${e?.message}`);
  }
}
async function askLaravel(appDomain, payload) {
  const r = await axios.post(`${appDomain}/api/viber/flow-node`, payload, { headers: nodeHeaders(), timeout: 60000, validateStatus: () => true });
  if (r.status >= 400) throw new Error(`flow-node HTTP ${r.status}: ${JSON.stringify(r.data)}`);
  return r.data || {};
}

// ── Graph helpers (identical to the LINE engine) ─────────────────────────────
...
function evalOneCondition(c, vars) {
  ...
  switch (op) {
    ...
    default:
      console.warn(`[VB-NODE] condition: unknown operator "${c?.operator}" — treated as FALSE`);
      return false;
  }
}
function evalCondition(d, vars) {
  ...
  console.log(`[VB-NODE] condition rules=${rules.length} -> ${met ? "TRUE (yes port)" : "FALSE (no port)"}`);
  return met;
}
...
async function walk(ctx, startId) {
  ...
  let current = startId;
  let guard = 0;
  console.log(`[VB-WALK] start flow=${flowId} user=${userId} from=${startId} nodes=${nodes.size}`);

  while (current && guard++ < 100) {
    const node = nodes.get(String(current));
    if (!node) { console.warn(`[VB-WALK] node id="${current}" NOT FOUND — ending flow=${flowId}`); break; }
    const type = String(node.type || "");
    const d = node.data || {};
    let port = "out";
    console.log(`[VB-NODE] → type=${type} id=${node.id} flow=${flowId}`);
    ...
    try {
      switch (type) {
        ...
        case "delay": {
          ...
          if (ms > 0) { console.log(`[VB-FLOW-NODE] delay node=${node.id} ${ms}ms`); await sleep(ms); }
          break;
        }
        case "condition":
        ...
        case "end":
          ...
          console.log(`[VB-FLOW-NODE] end flow=${flowId} user=${userId}`);
          return;
        default:
          console.warn(`[VB-FLOW-NODE] node type "${type}" has no executor — skipped (flow=${flowId} node=${node.id})`);
      }
    } catch (e) {
      console.error(`[VB-FLOW-NODE] node ${node.id} (${type}) failed: ${e?.message}`);
    }

    current = nextNode(flow, node.id, port);
  }

  if (guard >= 100) console.warn(`[VB-FLOW-NODE] walk hit the 100-node guard (flow=${flowId})`);
  clearSession(channelId, userId);
}
...
function park(ctx, nodeId) {
  ...
  console.log(`[VB-FLOW-NODE] parked at node=${nodeId} key=${key}`);
}
function clearSession(channelId, userId) { VB_SESSIONS.delete(sessionKeyFor(channelId, userId)); }
export const hasSession = (channelId, userId) => VB_SESSIONS.has(sessionKeyFor(channelId, userId));
export function pruneSessions(maxAgeMs = 86_400_000) {
  const cutoff = Date.now() - maxAgeMs; let n = 0;
  for (const [k, s] of VB_SESSIONS) if (s.parkedAt < cutoff) { VB_SESSIONS.delete(k); n++; }
  return n;
}
...
export async function runFlow({ auth, flow, userId, text, flowId, channelId, workspaceId, appDomain, vars }) {
  ...
  if (!start) { console.warn(`[VB-FLOW-NODE] flow ${flowId} has no trigger node`); return false; }
  const ctx = {
    auth, flow, userId, flowId, channelId, workspaceId, appDomain,
    vars: { text: String(text || ""), user_id: String(userId), ...(vars || {}) },
  };
  console.log(`[VB-FLOW-NODE] START flow=${flowId} channel=${channelId} user=${userId}`);
...
export function canResume(channelId, userId, text) {
  const sess = VB_SESSIONS.get(sessionKeyFor(channelId, userId));
...
export async function resumeFlow({ channelId, userId, text, vars }) {
  ...
  const sess = VB_SESSIONS.get(key);
  if (!sess) { console.log(`[VB-RESUME] no session key=${key}`); return false; }
  ...
  if (!parked) { console.warn(`[VB-RESUME] parked node "${sess.nodeId}" missing — dropping`); VB_SESSIONS.delete(key); return false; }
  ...
  VB_SESSIONS.delete(key);
  sess.vars.text = String(text || "");
  console.log(`[VB-FLOW-NODE] RESUME flow=${sess.flowId} from=${sess.nodeId} port=${port}`);
